# Capacity-matched PatchTST controls vs PFB
...
foreach ($seed in $seeds) {
    ...
    Invoke-CapMatchRun -Model "PFB-Direct" -Data "ETTh2" -RootPath $ettRoot -DataPath "ETTh2.csv" `
        -ModelId "CapMatch_MS$seed`_ETTh2_$ettPred" -SeqLen $ettSeq -LabelLen $ettLabel -PredLen $ettPred -EncIn $ettEnc -BatchSize $ettBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed

    Invoke-CapMatchRun -Model "PFB-Projected" -Data "ETTh2" -RootPath $ettRoot -DataPath "ETTh2.csv" `
        -ModelId "CapMatch_MS$seed`_ETTh2_$ettPred" -SeqLen $ettSeq -LabelLen $ettLabel -PredLen $ettPred -EncIn $ettEnc -BatchSize $ettBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed
    ...
    Invoke-CapMatchRun -Model "PFB-Direct" -Data "ETTm2" -RootPath $ettRoot -DataPath "ETTm2.csv" `
        -ModelId "CapMatch_MS$seed`_ETTm2_$ettPred" -SeqLen $ettSeq -LabelLen $ettLabel -PredLen $ettPred -EncIn $ettEnc -BatchSize $ettBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed

    Invoke-CapMatchRun -Model "PFB-Projected" -Data "ETTm2" -RootPath $ettRoot -DataPath "ETTm2.csv" `
        -ModelId "CapMatch_MS$seed`_ETTm2_$ettPred" -SeqLen $ettSeq -LabelLen $ettLabel -PredLen $ettPred -EncIn $ettEnc -BatchSize $ettBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed
}
...
foreach ($seed in $seeds) {
    ...
    Invoke-CapMatchRun -Model "PFB-Direct" -Data "custom" -RootPath $weatherRoot -DataPath "weather.csv" `
        -ModelId "CapMatch_MS$seed`_Weather_$weatherPred" -SeqLen $weatherSeq -LabelLen $weatherLabel -PredLen $weatherPred -EncIn $weatherEnc -BatchSize $weatherBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed

    Invoke-CapMatchRun -Model "PFB-Projected" -Data "custom" -RootPath $weatherRoot -DataPath "weather.csv" `
        -ModelId "CapMatch_MS$seed`_Weather_$weatherPred" -SeqLen $weatherSeq -LabelLen $weatherLabel -PredLen $weatherPred -EncIn $weatherEnc -BatchSize $weatherBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed
}
...
foreach ($seed in $seeds) {
    foreach ($h in $illHorizons) {
        ...
        Invoke-CapMatchRun -Model "PFB-Direct" -Data "custom" -RootPath $illRoot -DataPath "national_illness.csv" `
            -ModelId "CapMatch_MS$seed`_Illness_$h" -SeqLen $illSeq -LabelLen $illLabel -PredLen $h -EncIn $illEnc -BatchSize $illBatch `
            -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed

        Invoke-CapMatchRun -Model "PFB-Projected" -Data "custom" -RootPath $illRoot -DataPath "national_illness.csv" `
            -ModelId "CapMatch_MS$seed`_Illness_$h" -SeqLen $illSeq -LabelLen $illLabel -PredLen $h -EncIn $illEnc -BatchSize $illBatch `
            -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed
    }
}
