# Capacity-matched PatchTST controls vs PFB on Exchange
...
#   PFB-Direct target:                        3,256,768 params
#   Best match: d_model=192, e_layers=5, d_ff=512     3,283,456 params (0.82% off)
#
# Models: PatchTST_base, PatchTST_capacity, PFB-Direct, PFB-Projected
...
foreach ($seed in $seeds) {
    ...
    Invoke-CapMatchRun -Model "PFB-Direct" -Data "custom" -RootPath $dataRoot -DataPath "exchange_rate.csv" `
        -ModelId "CapMatch_MS$seed`_Exchange_$exPred" -SeqLen $exSeq -LabelLen $exLabel -PredLen $exPred -EncIn $exEnc -BatchSize $exBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed

    Invoke-CapMatchRun -Model "PFB-Projected" -Data "custom" -RootPath $dataRoot -DataPath "exchange_rate.csv" `
        -ModelId "CapMatch_MS$seed`_Exchange_$exPred" -SeqLen $exSeq -LabelLen $exLabel -PredLen $exPred -EncIn $exEnc -BatchSize $exBatch `
        -DModel $base_d_model -ELayers $base_e_layers -Dff $base_d_ff -Seed $seed
}
