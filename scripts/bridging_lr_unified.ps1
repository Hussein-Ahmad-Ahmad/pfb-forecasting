...
#   Original benchmark: PFB-Direct/PFB-Projected at LR=1e-3, PatchTST at LR=1e-3, DLinear at LR=1e-2
    ...
    function Get-RunArgs([string]$model, [string]$dataset, [int]$horizon, [int]$seed) {
        ...
        $modelArgs = switch ($model) {
            ...
            'PFB-Projected' {
                ...
                $fArgs = @('--model','PFB-Projected','--label_len',$ll,
