...
Extracts MS2021, MS2022, MS2023 results and computes meanÂ±std for publication
...
RESULTS_FILE = ROOT / 'results' / 'result_long_term_forecast.txt'
OUTPUT_FILE = ROOT / 'results' / 'multiseed_statistics.csv'

def parse_results_file(filename=None):
    """Parse the result file and extract multi-seed experiments"""
    filename = Path(filename) if filename is not None else RESULTS_FILE
    results = {}
    parsed_count = 0
    skipped_count = 0
    models_found = set()
...
def wilcoxon_tests(df):
    """Perform Wilcoxon signed-rank tests: PFB_v0 vs baselines"""
    datasets = df['Dataset'].unique()
    
    print("\n" + "="*80)
    print("WILCOXON SIGNED-RANK TESTS (PFB_v0 vs Baselines)")
    print("="*80)
    ...
    for dataset in datasets:
        pfb_row = df[(df['Model'] == 'PFB_v0') & (df['Dataset'] == dataset)]
    ...
    if pfb_mse and patchtst_mse:
        ...
        if p_value < 0.05:
            winner = "PFB_v0" if np.mean(pfb_mse) < np.mean(patchtst_mse) else "PatchTST"
    ...
    if pfb_mse and dlinear_mse:
        ...
        print(f"  Statistic: {stat:.4f}, p-value: {p_value:.6f}")
        if p_value < 0.05:
            winner = "PFB_v0" if np.mean(pfb_mse) < np.mean(dlinear_mse) else "DLinear"
            print(f"  [+] Significant difference (p<0.05): {winner} is better")
        else:
            print(f"  [-] No significant difference (p>=0.05)")
...
def print_publication_table(df):
    ...
    print("\n" + "="*80)
    print("PUBLICATION RESULTS TABLE (Mean Â± Std)")
    print("="*80)
    
    datasets = sorted(df['Dataset'].unique())
    models = ['PFB_v0', 'PatchTST', 'DLinear']
    ...
    for dataset in datasets:
        ...
        for model in models:
            ...
            if not row.empty:
                ...
                print(f"{mean:>15.4f}Â±{std:<4.4f}", end='')
            else:
                print(f"{'N/A':>20}", end='')
        print()
...
def main():
    ...
    print(f"Expected: 18 (3 models Ã— 6 datasets)")
