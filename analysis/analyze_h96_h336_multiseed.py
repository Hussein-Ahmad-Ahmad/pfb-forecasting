...
def parse_results(filename):
    ...
    while i < len(lines):
        ...
        # Extract model — try model_id pattern first: PFB-Direct_MS... or PFB-Projected_MS... or PatchTST_MS... or DLinear_MS...
        model = None
...
def write_markdown(summary_df, raw_df):
    ...
    for horizon in HORIZONS:
        for dataset in DATASETS:
            ...
            pfb_direct_row = sub[sub['model'] == 'PFB-Direct']
            ...
            pfb_mse = pfb_direct_row['mse_mean'].values[0] if not pfb_direct_row.empty else None
...
def main():
    ...
    for dataset in DATASETS:
        ...
        print(f'  {dataset}: PFB-Direct={vals.get("PFB-Direct", "?"):.4f}  '
              f'PFB-Projected={vals.get("PFB-Projected", "?"):.4f}  '
