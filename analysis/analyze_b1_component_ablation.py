...
COMPONENT_ORDER = ['PatchOnly', 'SerialRefinement', 'FusionOnly', 'Full']
COMPONENT_LABELS = {
    'PatchOnly': 'Backbone only',
    'SerialRefinement': 'Backbone + refinement (no fusion)',
...
def _parse_run(exp_name: str, mse: float, mae: float, source: str, line_no: int) -> Optional[ParsedRun]:
    match = re.search(r'B1Chain_MS(\d{4})_(ETTm2|ETTh2|Weather)_(\d+)_(PatchOnly|SerialRefinement|FusionOnly|Full)', exp_name)
    if not match:
        return None
    return ParsedRun(
...
def _count_params(component: str, enc_in: int) -> int:
    from models import PFB_Direct, PFB_PatchOnly, PFB_SerialRefinement

    if component == 'PatchOnly':
        model = PFB_PatchOnly.Model(_Args(enc_in=enc_in, pfb_k=0)).float()
    elif component == 'SerialRefinement':
        model = PFB_SerialRefinement.Model(_Args(enc_in=enc_in, pfb_k=3)).float()
    elif component == 'FusionOnly':
        model = PFB_Direct.Model(_Args(enc_in=enc_in, pfb_k=0)).float()
    elif component == 'Full':
        model = PFB_Direct.Model(_Args(enc_in=enc_in, pfb_k=3)).float()
    else:
