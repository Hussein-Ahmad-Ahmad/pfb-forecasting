...
def _parse_model(exp_name: str) -> Optional[str]:
    ...
    if "PFB-Direct" in exp_name:
        return "PFB-Direct"
    if "PFB-Projected" in exp_name:
        return "PFB-Projected"
...
def compute_param_counts(exps: List[ParsedExperiment]) -> Dict[Tuple[str, int, int, int, int, int], int]:
    ...
    """
    from models import PatchTST, PFB_Direct, PFB_Projected
...
def to_markdown_summary(df: pd.DataFrame) -> str:
    ...
    for (dataset, horizon), subset in df.groupby(["Dataset", "Horizon"], sort=True):
        ...
        pfb_direct = subset[subset["Model"] == "PFB-Direct"]
        lines.append("")
        if not pfb_direct.empty:
            reference = pfb_direct.iloc[0]
            for control in ["PatchTST_base", "PatchTST_capacity", "PatchTST_depth", "PFB-Projected"]:
                control_row = subset[subset["Model"] == control]
                if control_row.empty:
                    continue
                compare = control_row.iloc[0]
                lines.append(
                    "- PFB-Direct vs {model}: delta_mse={mse:+.4f}, delta_mae={mae:+.4f}".format(
