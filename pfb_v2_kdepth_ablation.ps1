# Experiment: K-depth refinement ablation for PatchFusionBERT_v2
# Mirrors pfb_v0_kdepth_ablation.ps1 but with PatchFusionBERT_v2 model.
# Runs:
#   - Datasets: ETTh2, Weather
#   - Horizons: 96, 192, 336
#   - K: 1, 2, 3
#   - Seed: single (2021) to match v0 ablation for direct comparison
...
function Invoke-KDepthV2Run {
    ...
    $modelId = "KDepthV2_MS$Seed`_${DatasetTag}_$PredLen`_K$K"
...
foreach ($h in $horizons) {
    foreach ($k in $ks) {
        Invoke-KDepthV2Run -Data "ETTh2" -RootPath $dataRoot -DataPath "ETTh2.csv" -DatasetTag "ETTh2" `
...
foreach ($h in $horizons) {
    foreach ($k in $ks) {
        Invoke-KDepthV2Run -Data "custom" -RootPath $dataRoot -DataPath "weather.csv" -DatasetTag "Weather" `
            -SeqLen 336 -LabelLen 96 -PredLen $h -EncIn 21 -BatchSize 8 -Seed $seed -K $k
    }
}

# Parse results immediately after training
Write-Host "Parsing v2 K-depth results..."
...
Write-Host "Done. Results saved to results_analysis/pfbv2_kdepth_ablation.csv"
