...
try {
    ...
    function Get-RunArgs([string]$model, [string]$dataset, [int]$horizon, [int]$seed) {
        ...
        if ($model -eq 'PatchFusionBERT_v0') { $modelId = "PFBv0_MS${seed}_${dataset}_${horizon}" }
        if ($model -eq 'PatchFusionBERT_v2') { $modelId = "PFBv2_MS${seed}_${dataset}_${horizon}" }
        ...
        $modelArgs = switch ($model) {
            'PatchFusionBERT_v0' {
                @('--model','PatchFusionBERT_v0','--label_len','96',
                  '--e_layers','3','--d_layers','1','--factor','3',
                  '--d_model','128','--d_ff','512','--n_heads','8',
                  '--patch_len','16','--stride','8')
            }
            'PatchFusionBERT_v2' {
                $fArgs = @('--model','PatchFusionBERT_v2','--label_len','48',
