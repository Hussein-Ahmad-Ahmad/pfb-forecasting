"""
PatchTST_LargeHead: large-head PatchTST control for the PFB study.

The model keeps the PatchTST patch embedding and encoder, then replaces the
single-layer FlattenHead with a two-layer MLP head:
    Flatten -> Linear(nf, d_ff_head) -> GELU -> Dropout -> Linear(d_ff_head, pred_len)

Purpose: test whether adding capacity to the output head can recover similar
accuracy-capacity behavior to PFB-Direct.

Head size: nf=5376 with d_ff_head=512 gives about 2.85M head parameters versus
the PatchTST base head of about 1.03M.
"""
